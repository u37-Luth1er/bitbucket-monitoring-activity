...
import json, random, sys, requests, pytz, argparse

class repo_updated_monitoring:
    def __init__(self, token, updated):
        self._token = token
        self.updated = updated
    def updated_monitoring(self):
        ...
        sheet.append(header)
        data_corte = datetime.strptime(self.updated, '%d/%m/%Y')
        ...
        for repo in repos_list:
            ...
            if updated_on > data_corte:
                ...
                sheet.append(row)
        workbook.save(filename="repo-update-recently.xlsx")
